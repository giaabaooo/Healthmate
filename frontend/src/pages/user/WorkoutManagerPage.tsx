...
const WorkoutManagerPage: React.FC = () => {
  ...
  const fetchWorkouts = useCallback(async () => {
    try {
      ...
      const response = await fetch('https://healthmate.onrender.com/api/workouts', {
        headers: {
  ...
  const fetchWorkoutLogs = useCallback(async () => {
    try {
      const savedToken = localStorage.getItem('token');
      const response = await fetch('https://healthmate.onrender.com/api/tracker/workouts?period=week', {
        headers: {
          'Authorization': `Bearer ${savedToken}`
        }
      });
  ...
  const handleSaveWorkout = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://healthmate.onrender.com/api/tracker/workouts', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDeleteLog = async (logId: string) => {
    ...
    try {
      const response = await fetch(`https://healthmate.onrender.com/api/tracker/workouts/${logId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
