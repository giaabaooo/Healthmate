...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://healthmate-y9vt.onrender.com/api/users/login', {
        method: 'POST',
        headers: {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const beResponse = await fetch('https://healthmate-y9vt.onrender.com/api/users/google-login', {
          method: 'POST',
