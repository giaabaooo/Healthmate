...
const MealPlannerPage = () => {
  ...
  useEffect(() => {
    const autoCalculateTarget = async () => {
        ...
        try {
            const res = await fetch('https://healthmate.onrender.com/api/goals/my-goal', { headers: getAuthHeaders() });
            if(res.ok) {
  ...
  useEffect(() => {
      const analyzeLimit = async () => {
          ...
          if (totalCalories >= calorieGoal && calorieGoal > 0 && totalCalories !== prevCal.current) {
              ...
              try {
                  const res = await fetch('https://healthmate.onrender.com/api/meal-plans/ai/analyze-calories', {
                      method: 'POST',
  ...
  const fetchMealPlan = async () => {
    try {
      setLoading(true);
      const res = await fetch(`https://healthmate.onrender.com/api/meal-plans/${selectedDate}`, { headers: getAuthHeaders() });
      const data = await res.json();
      setItems(data.items || []);
  ...
  const fetchFoods = async () => {
    try {
      const res = await fetch('https://healthmate.onrender.com/api/foods');
      const data = await res.json();
      setFoods(data);
  ...
  const fetchAiRecs = async (forceGenerate = false) => {
    ...
    try {
      ...
      const res = await fetch('https://healthmate.onrender.com/api/meal-plans/ai/recommend', { headers: getAuthHeaders() });
      const data = await res.json();
      setAiRecs(data || {});
  ...
  const addFoodToMealPlan = async (item: any, forceSlot?: MealSlot) => {
    try {
      ...
      const res = await fetch(`https://healthmate.onrender.com/api/meal-plans/${selectedDate}/items`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const updateQuantity = async (itemId: string) => {
    ...
    try {
      const res = await fetch(`https://healthmate.onrender.com/api/meal-plans/${selectedDate}/items/${itemId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
  ...
  const handleRemoveConfirm = async () => {
    try {
      const res = await fetch(
        `https://healthmate.onrender.com/api/meal-plans/${selectedDate}/items/${deleteModal.itemId}`,
