...
const AdminMealPlannerPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch('https://healthmate.onrender.com/api/users', { headers: getAuthHeaders() });
  ...
  const fetchMealPlan = async () => {
    try {
      ...
      const res = await fetch(`https://healthmate.onrender.com/api/meal-plans/${selectedDate}${q}`, { headers: getAuthHeaders() });
      const data = await res.json();
      setItems(data.items || []);
  ...
  const fetchWeeklyData = async () => {
    ...
    await Promise.all(week.map(async (date) => {
      try {
        const q = targetUserId ? `?target_user_id=${targetUserId}` : '';
        const res = await fetch(`https://healthmate.onrender.com/api/meal-plans/${date}${q}`, { headers: getAuthHeaders() });
        const data = await res.json();
  ...
  const handleRemoveConfirm = async () => {
    try {
      ...
      const res = await fetch(
        `https://healthmate.onrender.com/api/meal-plans/${selectedDate}/items/${deleteModal.itemId}${q}`,
