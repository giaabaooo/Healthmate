...
const MealPlannerSection: React.FC<MealPlannerSectionProps> = ({ onBack }) => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch('https://healthmate.onrender.com/api/users', { headers: authHeaders() });
  ...
  const fetchMealPlan = async () => {
    try {
      ...
      const res = await fetch(`https://healthmate.onrender.com/api/meal-plans/${selectedDate}${q}`, { headers: authHeaders() });
      const data = await res.json();
      setItems(data.items || []);
  ...
  const fetchWeeklyData = async () => {
    ...
    await Promise.all(week.map(async (date) => {
      try {
        const q = targetUserId ? `?target_user_id=${targetUserId}` : '';
        const res = await fetch(`https://healthmate.onrender.com/api/meal-plans/${date}${q}`, { headers: authHeaders() });
        const data = await res.json();
  ...
  const deleteItem = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://healthmate.onrender.com/api/meal-plans/${selectedDate}/items/${deleteTarget.id}${q}`,
