...
const FoodFormModal = ({ isOpen, foodId, onClose, onSuccess }: FoodFormModalProps) => {
  ...
  const fetchFood = async (id: string) => {
    ...
    try {
      const response = await fetch(`https://healthmate-y9vt.onrender.com/api/foods/${id}`);
      ...
      if (response.ok) {
        ...
        if (data.image) {
          setImagePreview(`https://healthmate-y9vt.onrender.com${data.image}`);
        }
      } else {
    ...
    if (!formData.name.trim()) {
      ...
      try {
        const res = await fetch(
          `https://healthmate-y9vt.onrender.com/api/foods?search=${encodeURIComponent(formData.name.trim())}`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = isEditMode
      ? `https://healthmate-y9vt.onrender.com/api/foods/${foodId}`
      : 'https://healthmate-y9vt.onrender.com/api/foods';
